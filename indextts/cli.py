...
def main():
    ...
    parser.add_argument("-m", "--model_dir", type=str, default="checkpoints", help="Path to the model directory. Default is 'checkpoints'")
    parser.add_argument("--fp16", action="store_true", default=True, help="Use FP16 for inference if available")
    parser.add_argument("--no-fp16", action="store_false", dest="fp16", help="Disable FP16 for inference")
    ...
    tts.infer(audio_prompt=args.voice, text=args.text.strip(), output_path=output_path, verbose=args.verbose)
